...
| --- | --- | --- | --- | --- |
| `repo_scaffold_present` | Smoke | none | Prove the implementation skeleton exists | Root docs, `src/backend/Sysml2Editor.Api`, `src/frontend`, `src/shared/contracts/generated`, `tests/unit`, `tests/integration`, `tests/e2e`, `fixtures/`, and `scripts/` are checked in and match the repo layout in [README.md](../../README.md) |
