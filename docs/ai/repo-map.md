# Repository Map

Use [plan.md](../../plan.md) for the full documentation index.

## Intended Source Layout

The setup target is:

```text
src/backend/
src/frontend/
src/shared/
tests/
fixtures/
docs/ai/
docs/implementation/
```

## How To Use The Docs During Implementation

- Start with the AI guide and the readiness checklist.
- Use the decision docs as the contract for implementation work.
- Use the structure doc when creating or rearranging folders.
- Use the test matrix before adding new behavior.
- Use the write policy when deciding where a change belongs.

## Update Rule

If you add a new root-level document or major setup folder, update this map and [plan.md](../../plan.md). Update the README only for new top-level entry points.
